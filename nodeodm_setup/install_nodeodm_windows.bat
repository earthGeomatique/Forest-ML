...
echo.
echo [5/5] Creation du script de demarrage...
...
(
    ...
    echo   -p 3000:3000 ^
    echo   -v "%DATA_DIR%":/var/www/data ^
    echo   --restart unless-stopped ^
    echo   opendronemap/nodeodm:latest
    echo echo.
    echo echo NodeODM demarre sur http://localhost:3000
    echo echo Appuyez sur une touche pour fermer cette fenetre...
    echo pause ^>nul
) > "%START_SCRIPT%"
...
(
    ...
    echo echo NodeODM arrete.
    echo pause
) > "%STOP_SCRIPT%"

echo    OK : Scripts crees dans %USERPROFILE%\ForestDL\

:: ── Démarrer NodeODM maintenant ──────────────────────────────────────────
echo.
echo ============================================================
...
if %errorLevel% EQU 0 (
    echo.
    echo ============================================================
    echo   ✓  NodeODM est installe et demarre avec succes !
    echo ============================================================
    echo.
    echo   URL a utiliser dans ForestDL : http://localhost:3000
    echo   Token                        : (laisser vide)
    echo.
    echo   Pour demarrer NodeODM apres redemarrage :
    echo   %START_SCRIPT%
    echo.
    echo   Pour arreter NodeODM :
    echo   %STOP_SCRIPT%
    echo ============================================================
) else (
    echo [ATTENTION] NodeODM demarre mais pas encore pret.
    echo Attendez 30 secondes puis testez : http://localhost:3000/info
)
